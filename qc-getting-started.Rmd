...
```{r,}
test <- data.frame( y= assay(BRD4df)[10,1:18], x = rep(c(0,15,60,600,3600,14000), each = 3))
res <- loess(y ~ x, data = test)
smoothed <- predict(res)
plot(res)
lines(smoothed, x = test$x, lwd = 2, col = "blue")
res <- isoreg(x = test$x, y = test$y)
plot(res)

ideastat <- vector(mode = "numeric", length = nrow(assay(BRD4df_filtered_imputed)))
for (j in seq.int(nrow(assay(BRD4df_filtered_imputed)))){
    ...
    ideastat[j] <- sum(abs(order(res2$Mean, decreasing = TRUE) - order(res2$x, decreasing = TRUE)))
}
plot(ideastat, lwd = 1)
 plot(table(ideastat), type = "h", lwd = 2, col = "red")
 pmult <- table(ideastat)/length(ideastat)
 cumsum(pmult)
...
test <- data.frame( y= c(assay(BRD4df_filtered_imputed)[ch, 19:36]), x = rep(c(0, 15, 60, 600, 3600, 14000), each = 6), z = rep(chstate[ch], times = 18))

test |> ggplot(aes(x = x, y = y, col = z)) + geom_point() + theme_classic()

cor(assay(BRD4df_filtered_imputed)[ch, 1:18][1,], assay(BRD4df_filtered_imputed)[ch, 1:18][2,])
cor(assay(BRD4df_filtered_imputed)[ch, 19:36][1,], assay(BRD4df_filtered_imputed)[ch, 19:36][2,])
...
```{r,}
num_exch <- function(peptides){
    
    ne <- vapply(strsplit(peptides, split = ""),
                 function(x) length(x) - 3, numeric(1))
    return(ne)
}
ne <- num_exch(peptides = rownames(assay(BRD4df_filtered_imputed)))

check_max_deuteration <- function(object, numExch){
    
    wh <- which(assay(object) > numExch, arr.ind = TRUE)
    return(wh)
    
}
check_max_deuteration(object = BRD4df_filtered_imputed, numExch = ne)
```
```{r,}
pep <- sapply(strsplit(rownames(assay(BRD4df_filtered_imputed)), split = ""), function(x) x[seq.int(length(x) - 1)])
pep <- sapply(pep, function(x) paste(x, sep = "", collapse = "") )
```
