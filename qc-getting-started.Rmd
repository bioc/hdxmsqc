...
```
```{r,}
hdxsite2 <- read.csv("inst/ELN55049_AllResultsTables_Uncurated_HDSite.txt", skip = 1, sep = "\t", header = FALSE)
hdxsite2_names <- read.csv("inst/ELN55049_AllResultsTables_Uncurated_HDSite.txt", nrows = 1, sep = "\t", header = FALSE)
hdxsite2 <- data.frame(hdxsite2)
colnames(hdxsite2)[seq_along(hdxsite2_names)] <- hdxsite2_names[1,]
colnames(hdxsite2)[which(colnames(hdxsite2) == "NA")] <- paste0("m+", which(colnames(hdxsite2) == "NA")-15)
```
...
BRD4uncurated_wc <- data.frame(BRD4uncurated[!duplicated(BRD4uncurated[,1:7]),])


spd <- Spectra(sps)
spd$Sequence <- BRD4uncurated_wc |> filter(Protein.State %in% c("+iBET-151", "apo BRD4(1-477) wt")) |> pull(Sequence)
